...
def check_pbc_vec(ref_pos_vec, pos_vec, box_vec):
    """Check periodicity of system and move atom positions
    ...
    pbc_pos_mat : float[:,:], shape = (num_atoms, 3)
    """
    pbc_pos_vec = np.copy(pos_vec)
    
    for i in range(3):
        if pos_vec[i] - ref_pos_vec[i] > box_vec[i]/2:
            pbc_pos_vec[i] = pos_vec[i] - box_vec[i]
        elif ref_pos_vec[i] - pos_vec[i] > box_vec[i]/2:
            pbc_pos_vec[i] = pos_vec[i] + box_vec[i]
